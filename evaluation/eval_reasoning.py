...
class Evaluator:
    ...
    async def evaluate_single_case(
        ...
        ground_truth = test_case["ground_truth_reasoning"]
        answer = test_case["answer_reasoning"]
        ...
        async with eval_semaphore:
            ...
            try:
                ...
                result = {
                    ...
                    "metrics": {
                        "reasoning_recall": float(scores_row.get("reasoning_recall", 0))
                    },
    ...
    def _export_to_csv(self, results: List[Dict[str, Any]]) -> Path:
        ...
        CSV Format:
            ...
            - reasoning_recall: reasoning recall (0-1)
            - ragas_score: Overall RAGAS score (0-1)
            - timestamp: When evaluation was run
        """
        dataset_name = self.test_dataset_path.stem
        csv_path = (
            self.results_dir / f"eval_reasoning_{dataset_name}.csv"
        )
                ...
                writer.writerow(
                    {
                        ...
                        "reasoning_recall": f"{metrics.get('reasoning_recall', 0):.4f}",
    ...
    def _display_results_table(self, results: List[Dict[str, Any]]):
        ...
        for result in results:
            ...
            if metrics:
                ...
                reasoning_recall = metrics.get("reasoning_recall", 0)
                ...
                logger.info(
                    ...
                    test_num,
                    question_display,
                    self._format_metric(reasoning_recall, 6),
    ...
    def _calculate_benchmark_stats(
        ...
        metrics_data = {
            "reasoning_recall": {"sum": 0.0, "count": 0},
        ...
        for result in valid_results:
            ...
            reasonning_recall = metrics.get("reasoning_recall", 0)
            if not _is_nan(reasonning_recall):
                metrics_data["reasoning_recall"]["sum"] += reasonning_recall
                metrics_data["reasoning_recall"]["count"] += 1
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        json_path = (
            self.results_dir
            / f"eval_reasoning_{dataset_name}.json"
        )
        ...
        logger.info("Average ReasoningRecall:      %.4f", avg["reasoning_recall"])
        logger.info("%s", "-" * 70)
        logger.info(
            "Min RAGAS Score:           %.4f",
